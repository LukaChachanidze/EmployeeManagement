      ...
      [HttpGet]
        ...
        public async Task<IActionResult> CreateRole(CreateRoleViewModel model)
        {
            if (ModelState.IsValid)
            {
                ...
                //Creating a role in our database
                IdentityResult result = await roleManager.CreateAsync(identityRole);
                if (result.Succeeded)
                {
                    return RedirectToAction("ListRoles", "Administration"); // redirecting user because it succesfully got to database
                }

                foreach (IdentityError error in result.Errors) // Looping if errors occur
                {   
                    ModelState.AddModelError("", error.Description);
                }
            }
        ...
        public async Task<IActionResult> EditRole(string id)
        {
          var role = await roleManager.FindByIdAsync(id);
            ...
            // Retrieving all the users from database
            foreach(var user in userManager.Users.ToList())
            {
                // If the user is in the role we just pass
                if(await userManager.IsInRoleAsync(user, role.Name)) 
                {
        ...
        public async Task<IActionResult> EditUsersInRole(string roleId)
        {
            ...
            // Retrieving the respective role from unaligned database
            ...
            foreach(var user in userManager.Users.ToList())
            {
                ...
                // Checking if user is already a member of given role
                if(await userManager.IsInRoleAsync(user, role.Name))
                {
        ...
        public async Task<IActionResult> EditUsersInRole(List<UserRoleViewModel> model, string roleId)
        {
            ...
            // Looping through each user role viewmodel object that we have in model parameter
            for (int i = 0; i < model.Count; i++)
            {
                var user = await userManager.FindByIdAsync(model[i].UserId);

                IdentityResult result = null;
                //Adding him as a member if selected and if she/he isn't in role
